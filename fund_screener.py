...
MAX_FEE = 2.5  # 管理费 ≤ 2.5% (此变量仅用于信息展示，不用于筛选)
...
def main():
    ...
    # 不再对场外基金进行预筛选，直接处理所有基金代码
    print(f"已加载 {len(fund_list)} 个基金代码，开始处理所有基金。", flush=True)
    ...
    for code in fund_list:
        ...
        if (metrics['annual_return'] >= MIN_RETURN and
            ...
            metrics['sharpe'] >= MIN_SHARPE): # 删除了管理费的筛选条件
            result = {
                '基金代码': code,
                '基金名称': name,
                '年化收益率 (%)': metrics['annual_return'],
        ...
        debug_data.append(debug_info)
        print("-" * 50, flush=True)
        time.sleep(random.uniform(8, 15))  # 随机延时 0.5-1 秒
