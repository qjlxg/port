...
def get_fund_net_values(code, start_date, end_date):
    ...
    headers = {
        ...
        'Referer': f'http://fund.eastmoney.com/{code}.html'  # 添加 Referer 头
    }
    try:
        ...
        print(f"获取基金 {code} 历史净值失败：响应内容不是有效的 JSON。")
        print(f"响应内容前200字符：{response.text[:200] if 'response' in locals() else '无响应'}...")
        return pd.DataFrame(), None, '天天基金网'
