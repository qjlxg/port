...
def get_fund_holdings(code):
    ...
    headers = {
        ...
        'Accept': 'text/javascript, application/javascript, */*',
        'X-Requested-With': 'XMLHttpRequest',
        'Cookie': 'st_si=123456789; st_asi=delete; fund_risk_tip=1'
    }
    try:
        response = session.get(url, headers=headers, timeout=TIMEOUT)
        response.raise_for_status()
    ...
    try:
        ...
        print(f"    × FundShareHolding 接口获取持仓失败: {e}，尝试 ccmx 接口。", flush=True)
    ...
    try:
        ...
        table = soup.find('div', id='cctable')
        if table:
        ...
        print(f"    × ccmx 接口获取持仓失败: {e}", flush=True)
        return []
    finally:
