...
def get_fund_net_values(code):
    """
    从 fund.eastmoney.com/pingzhongdata/ 接口获取基金历史净值数据。
    """
    ...
    try:
        ...
        # 提取净值数据
        net_worth_match = re.search(r'Data_netWorthTrend\s*=\s*(\[.*?\]);', response.text)
        ...
        # 转换为 DataFrame
        df = pd.DataFrame(net_worth_list, columns=['date', 'net_value', 'daily_return', 'unknown'])
...
def main():
    ...
    for idx, row in funds_df.iterrows():
        ...
        # 获取历史净值数据
        net_df = get_fund_net_values(code)
